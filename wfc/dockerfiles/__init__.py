...
:mod:`wfc.dockerfiles.conda`, :mod:`wfc.dockerfiles.inherit`, and
:mod:`wfc.dockerfiles.byo`. The :func:`generate_for_backend` dispatch
...
from . import byo, conda, inherit, pixi
...
def generate_for_backend(backend: str, **kwargs) -> Optional[str]:
    ...
    if backend == "inherit":
        return inherit.generate(**kwargs)
    if backend == "byo":
