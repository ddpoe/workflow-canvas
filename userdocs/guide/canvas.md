<!-- generated from pm_mvp::docs.consumer.canvas @ 9f0f320a9a4e; do not edit -->
...
**Rules:**
- Variables are created **only** in the Pipeline Variables panel (not inline from the bind picker — D-6).
...
When you open a past pipeline from the History tab via "Open in canvas," the canvas tries to fetch `pipeline.editable.json` first (falling back to `pipeline.json` for legacy runs). If the editable sidecar is present, `parsePipelineJSON` repopulates the `pipelineVariables` store and re-enters each bound row's actor in the `bound` state — so the Pipeline Variables panel and all binding chips reappear exactly as they were at submission time. No retyping required.
