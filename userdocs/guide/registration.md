<!-- generated from pm_mvp::docs.consumer.registration @ 9e2e6d7b7ded; do not edit -->
...
A **method** is a single analysis step -- a public Python function that lives in a directory under a module. (The function may optionally use the `@wfc_method` decorator, but plain public functions are discovered and tracked all the same.)
...
- The `@wfc_method` decorator is optional. AST discovery picks up every public (non-underscore) function in the script automatically; `@wfc_method` is ergonomic Tier-1 sugar (it lets the scanner strip `inputs`/`params` injection arguments), not a requirement for a function to be discovered or tracked.
